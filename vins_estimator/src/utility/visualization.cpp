...
void printStatistics(const Estimator &estimator, double t)
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        {
            ofstream onlineCalibT(EX_CALIB_T_PATH, ios_base::app | ios::out | ios::binary);
            onlineCalibT << estimator.tic[i].transpose() << std::endl;
            onlineCalibT.close();
            ofstream onlineCalibR(EX_CALIB_R_PATH, ios_base::app | ios::out | ios::binary);
            onlineCalibR << Utility::R2ypr(estimator.ric[i]).transpose() << std::endl;
            onlineCalibR.close();
            ofstream ba(BA_PATH, ios_base::app | ios::out | ios::binary);
            ba << estimator.Bas[WINDOW_SIZE].transpose() << std::endl;
            ba.close();
            ofstream bg(BG_PATH, ios_base::app | ios::out | ios::binary);
            bg << estimator.Bgs[WINDOW_SIZE].transpose() << std::endl;
            bg.close();
            ofstream vs(VS_PATH, ios_base::app | ios::out | ios::binary);
            vs << estimator.Vs[WINDOW_SIZE].transpose() << std::endl;
            vs.close();
            ofstream ori(ORI_PATH, ios_base::app | ios::out | ios::binary);
            ori << Utility::R2ypr(estimator.Rs[WINDOW_SIZE]).transpose() << std::endl;
            ori.close();
